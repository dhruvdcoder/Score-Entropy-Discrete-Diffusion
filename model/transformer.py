...
from .utils import get_autocast_dtype
...
class DDiTBlock(nn.Module):
    ...
    def __init__(self, dim, n_heads, cond_dim, mlp_ratio=4, dropout=0.1):
        ...
        self.dropout1 = nn.Dropout(dropout)  # not used
...
class SEDD(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, indices, sigma):
        ...
        # breakpoint()
        # at each position in the sequence, set the score of the input token to 0
        # because the summation in the loss later will want to skip it.
        x = torch.scatter(
            x, -1, indices[..., None], torch.zeros_like(x[..., :1])
        )

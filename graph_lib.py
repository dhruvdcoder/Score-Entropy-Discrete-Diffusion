...
class Uniform(Graph):
    ...
    def transition(self, i, sigma):
        ...
        trans = trans.scatter(
            -1, i[..., None], 1 - trans.sum(dim=-1, keepdim=True)
        )
        return trans
    ...
    def score_entropy(self, score, sigma, x, x0):
        esigm1 = torch.where(
            sigma < 0.5, torch.expm1(sigma), torch.exp(sigma) - 1
        )
        ...
        neg_term = torch.where(
            x == x0,
            ratio * neg_term,
            torch.gather(score, -1, x0[..., None]).squeeze(-1) / esigm1
            + neg_term,
        )
...
class Absorbing(Graph):
    ...
    def transp_transition(self, i, sigma):
        ...
        edge += torch.where(
            i == self.dim - 1, 1 - (-sigma).squeeze(-1).exp(), 0
        )[..., None]
        return edge
    ...
    def score_entropy(self, score, sigma, x, x0):
        """
        Args:
            score: Tensor for shape (bsz, seq_len, vocab_size+1)
                At training time, from the network's side the score is architecturally equivalent to
                the usual 'logit' in cross-entropy based training,
                from the loss side it stands for $\log s() $
                but in reality it is
            sigma: Tensor for shape (bsz, 1)
            x: Tensor for shape (bsz, seq_len), i.e., x_t
                Observation: Contains many mask token indices as expected
                for absorbing state diffusion.
            x0: Tensor for shape (bsz, seq_len).
                Observation: non-masked tokens are the same as x, again,
                as expected for absorbing state diffusion.
        Returns:
            entropy: Tensor for shape (bsz, seq_len) with non-zero values only for mask tokens.
        """
        # breakpoint()
        rel_ind = x == self.dim - 1  # shape (bsz, seq_len)
        # rel_ind contains True where x contains the absorbing token (mask token) index.
        esigm1 = torch.where(
            sigma < 0.5, torch.expm1(sigma), torch.exp(sigma) - 1
        )

        ratio = (
            1 / esigm1.expand_as(x)[rel_ind]
        )  # shape: (total_absorbing_tokens,), where total_absorbing_tokens is the number of mask tokens in the batch.
        other_ind = x0[
            rel_ind
        ]  # shape: (total_absorbing_tokens,), contains the answers for the mask tokens

        # negative_term
        # Explanation:
        # score[rel_ind] has shape (total_absorbing_tokens, vocab_size+1)
        # because score has shape (bsz, seq_len, vocab_size+1) and rel_ind has shape (bsz, seq_len)
        # More explicit way to write this is:
        # idx = rel_ind.nonzero(as_tuple=True) # Convert boolean mask to indices
        # explicit_score = score[idx[0], idx[1], :]
        # explicit_score == score[rel_ind]
        #
        # torch.gather(score[rel_ind], -1, other_ind[..., None]).squeeze(-1) has shape (total_absorbing_tokens,)
        # where the i-th element is the score (or logit) of the answer for the i-th mask token in the batch.
        # the ratio multiplies the score (unorm_log_prob?) by a value based on sigma for that sequence.
        # The ratio is close to 1 for small values of t.
        # the neg-term is very similar to the first of the two terms in the cross-entropy loss but with a weight.
        neg_term = ratio * torch.gather(
            score[rel_ind], -1, other_ind[..., None]
        ).squeeze(
            -1
        )  # shape: (total_absorbing_tokens,)

        # positive term
        # the positive term replaces the normalizer in the cross-entropy loss.
        # What is most surprising is that it is a sum of exps, which can easily dominate the neg-term.
        pos_term = score[rel_ind][:, :-1].exp().sum(dim=-1)

        # constant term
        const = ratio * (ratio.log() - 1)
        # Why do they have constant in the loss?
